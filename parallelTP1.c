...
int neighboursMatrix(int i, int j){
    ...
    sum = matrix[i-1][j] + matrix[i-1][j+1] + matrix[i][j+1] + matrix[i+1][j+1] + matrix[i+1][j] + matrix[i+1][j-1] + matrix[i][j-1] + matrix[i-1][j-1];
...
int neighboursImg(int i, int j){
    ...
    //Soma todos os vizinhos de P1; 
    sum = img[i-1][j] + img[i-1][j+1] + img[i][j+1] + img[i+1][j+1] + img[i+1][j] + img[i+1][j-1] + img[i][j-1] + img[i-1][j-1];
        ...
        for(i=1; i<=endlines; i++) {
            for(j=1; j<=endcols; j++) {
                ...
                if(matrix[i][j]==1){
                    ...
                    //Vai a cada posição da matriz e testa as condições do algoritmo, caso se verifique, coloca essa posição a 0;
                    //(!p4 && p6 && p2) && !(p4 && p6 && p8)

                    if(!(matrix[i][j+1] && matrix[i+1][j] && matrix[i-1][j]) && !(matrix[i][j+1] && matrix[i+1][j] && matrix[i][j-1]) && neighboursMatrix(i, j) && transitionsMatrix(i, j)){
        ...
        for(i=1; i<=endlines; i++) {
            for(j=1; j<=endcols; j++) {
                ...
                if(img[i][j]==1){
                    ...
                    if(!(img[i][j+1] && img[i-1][j] && img[i][j-1]) && !(img[i-1][j] && img[i+1][j] && img[i][j-1]) && neighboursImg(i, j) && transitionsImg(i, j)){
                        matrix[i][j] = 0;
                        count++;
                    }
